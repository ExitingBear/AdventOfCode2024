...
```{r}
p1<-run_ops(c(2,4,1,5,7,5,1,6,0,3,4,3,5,5,3,0),c(34615120,0,0))
...
```{r}
run_ops(c(2,4,1,5,7,5,1,6,0,3,4,3,5,5,3,0),c(105706277661082,0,0))
```
...
So, B&C get reset every run (so they do not matter) only A does. Every run, A drops its last three bits. And so it can be worked backwards, the first three bits of A determine the last output, the second three bits determine the second to last, & so on.

So:
```{r}
### this is the target
target<-c(2,4,1,5,7,5,1,6,0,3,4,3,5,5,3,0)
### starting at 0
finala<-c(0)
...
for(i in 1:16){
  ...
  for(j in 1:length(finala)){
    ...
    check<-sapply(0:7,function(x){(run_ops(c(2,4,1,5,7,5,1,6,0,3,4,3,5,5,3,0),c(maybea*8+x,0,0)))[1]})
